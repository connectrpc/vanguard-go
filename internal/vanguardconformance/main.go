...
func Run(ctx context.Context, args []string, osStdin io.ReadCloser, osStdout, osStderr io.WriteCloser) error {
	...
	go func() {
		log.Println("Running conformance server\n\tgo run connectrpc.com/conformance/cmd/referenceserver")
		err := cmd.Run()
		execDone <- err
	...
	b, _ := protojson.MarshalOptions{Multiline: true}.Marshal(&resp)
	log.Printf("ServerCompatResponse<-: %s\n", b)
	...
	defer log.Println("Done creating proxy server")
	...
	b, _ = protojson.MarshalOptions{Multiline: true}.Marshal(&resp)
	log.Printf("<-ServerCompatResponse: %s\b", b)

	if err := encodeMessage(osStdout, &resp); err != nil {
		return fmt.Errorf("failed to encode response: %w", err)
	}

	log.Printf("Serving reference server on %s\n", refURL)
