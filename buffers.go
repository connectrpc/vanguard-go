...
// codecsEqual reports whether two codecs are equivalent.
func codecsEqual(left, right Codec) bool {
	...
	if eq, ok := left.(equaler); ok {
		return eq.Equal(right)
	}
	return left.Name() == right.Name()
}

// compressorsEqual reports whether two compressors are equivalent.
func compressorsEqual(left, right compressor) bool {
	if left == nil && right == nil {
		return true
	}
	if left == nil || right == nil {
